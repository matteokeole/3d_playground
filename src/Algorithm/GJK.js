...
export class GJK {
	 ...
	 */
	static support(hull1, hull2, D) {
		const s1 = new Vector3(hull1.getGeometry().support(D, hull1.getWorld()));
		const s2 = hull2.getGeometry().support(new Vector3(D).negate(), hull2.getWorld());
	 ...
	 * @param {Hull} hull2
	 */
	static test3d(hull1, hull2) {
		const D = new Vector3(1, 0, 0);
		const a = GJK.support(hull1, hull2, D);
